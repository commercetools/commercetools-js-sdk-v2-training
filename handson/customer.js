...
const getCustomerById = (ID) => apiRoot.withProjectKey({projectKey}).customers().withId({ID}).get().execute();

const getCustomerByKey = (key) => apiRoot.withProjectKey({projectKey}).customers().withKey({key}).get().execute();
...
const createCustomer = (customerData) => apiRoot.withProjectKey({projectKey})
                                                .customers().post({
...
const createCustomerKeyVerfiedEmail = (customerData) => apiRoot.withProjectKey({projectKey})
                                                                .customers().post({
                                                                    body:{
...
const assignCustomerToCustomerGroup = (customerKey,customerGroupKey) =>{
  ...
  return getCustomerByKey(customerKey).then(customer => {
    ...
    .customers().withId({ID:customer.body.id}).post({
      body:{
